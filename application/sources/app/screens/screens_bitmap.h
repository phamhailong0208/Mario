#ifndef __SCREENS_BITMAP_H__
#define __SCREENS_BITMAP_H__

#include "view_render.h"

// scr_peashooter
extern const unsigned char PROGMEM bitmap_mario [];
extern const unsigned char PROGMEM bitmap_mario_small [];
extern const unsigned char PROGMEM bitmap_mushroom [];
extern const unsigned char PROGMEM bitmap_block_brick [];
extern const unsigned char PROGMEM bitmap_block_question [];
extern const unsigned char PROGMEM bitmap_mushroom_super [];
extern const unsigned char PROGMEM bitmap_block_question_blink [];
extern const unsigned char PROGMEM bitmap_fire_flower [];
extern const unsigned char PROGMEM bitmap_mario_fire [];
extern const unsigned char PROGMEM bitmap_bullet [];
extern const unsigned char PROGMEM bitmap_boss [];
extern const unsigned char PROGMEM bitmap_warning [];
extern const unsigned char PROGMEM bitmap_pipe [];
#endif //__SCREENS_BITMAP_H__