                ...
                if ((SIZE_BITMAP_BLOCK_X > abs(b.x -camera_x - mario.x))) {
                    if (mario.on_ground == 0) {
                        if (b.type == BLOCK_QUESTION && b.active == true) {
                            if (mario.state == SMALL) task_post_pure_msg(MA_GAME_MUSHROOMSUPER_ID, MA_GAME_MUSHROOMSUPER_SPAWN);
                            else if (mario.state == BIG) {
                                task_post_pure_msg(MA_GAME_FIREFLOWER_ID, MA_GAME_FIREFLOWER_SPAWN);
                            }
                            b.active = false;
                        }
                    }
...
void ma_game_block_respawn(){
    for (uint8_t i = 0; i < NUM_BLOCKS; i++) {
        ...
        // block[i].x += 200;
        block[i].x = camera_x + BLOCK_X_INTERVAL + i * SIZE_BITMAP_BLOCK_X;
        block[i].y = block[i].spawn_y;
        if (mario.state == BIG || mario.state == FIRE) {
            block[i].y -= 5;
        }
    }
}

void ma_game_block_reset() {
    block[0].x = block[1].x = block[2].x = 0;
    block[0].y = block[1].y = block[2].y = 0;
    block[0].visible = block[1].visible = block[2].visible = BLACK;
    block[0].active = block[1].active = block[2].active = false;
}
...
void ma_game_block_handler(ak_msg_t* msg) {
    ...
    case MA_GAME_BLOCK_RESPAWN: {
        ...
        break;
    case MA_GAME_BLOCK_RESET: {
        APP_DBG_SIG("MA_GAME_BLOCK_RESET\n");
        ma_game_block_reset();
    }   
        break;
    default:
